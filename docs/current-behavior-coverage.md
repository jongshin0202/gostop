...
## Migration Step 6H Three-Ppeok coverage

The engine now checks the established `ppeoks >= 3` terminal condition immediately after authoritative Ppeok/Ssa-da stack creation and count increment. It keeps the seven-point base, leaves `specialWinner` unchanged, consumes Nagari carry into a structured terminal result, and emits `threePpeokDeclared` plus `handEnded` only after `ppeokFormed` and `specialResolved`.

Fixtures cover first/second/third and restored-above-threshold counts, both neutral players, carry powers zero through three, exact event order, lossless serialization, terminal action rejection, and viewer-safe hand/deck redaction. The classified and retained fallback browser paths both submit to engine authority; presentation alone preserves the Ppeok sound, 450ms delay, WIN label, reason, and final points. The former `finishSpecial` browser mutation path has been removed.

## Migration Step 6I final authority audit
...
## Second manual-preview regression pass
