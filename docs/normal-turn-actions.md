...
Before normal turn interaction, `resolveOpeningState` performs authoritative Chongtong resolution and then creates player-private opening triple decisions. A triple without its floor fourth offers SHAKE/KEEP SECRET; an immediately Bomb-ready triple offers SHAKE/BOMB and Bomb executes without a second confirmation. The engine advances decisions for both neutral players before normal play.

The existing human/AI controllers still select cards and targets, display private choices, choose AI policy, and preserve pacing. `declareShake` remains a public ×2 settlement declaration. Bomb atomically reveals/captures the four-card month, transfers exactly one Pi, grants two optional blanks, and enters deck draw, but contributes no settlement multiplier.

An accepted Shake records the exact three revealed card IDs in the actor's serializable `revealedShakeSets` and includes them in the public event. Both viewer projections can therefore review a declared Shake even after its cards leave the hand; KEEP SECRET produces neither public IDs nor a public event.

After all capture, Pi-transfer, and Sweep mutations are complete, `evaluateGoStop` evaluates the established seven-point threshold and strict improvement over the actor's last accepted GO score. An eligible turn pauses at a JSON-safe, player-private `goStopDecision`; otherwise authority either hands the turn to the other neutral player or reports that Nagari resolution is required. `declareGo` increments the actor's GO count, records the accepted score, emits public `goDeclared`, and performs the same handoff. `declareStop` sets the neutral ordinary winner, stores the structured `calculateSettlement` result, and emits public `stopDeclared` and `handEnded`. The browser only chooses AI preference and presents accepted events.
