...
(()=>{
  ...
  async function loadRankings(){
    const params=new URLSearchParams({metric:$('rankingMetric').value,limit:'250'});appendRange(params);if($('rankingMode').value)params.set('mode',$('rankingMode').value);
  ...
  $('applyRange').addEventListener('click',refreshCurrent);$('clearRange').addEventListener('click',()=>{$('filterFrom').value='';$('filterTo').value='';refreshCurrent();});
