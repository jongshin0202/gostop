...
export class FinalRankedRoomCore extends RankedRoomCore{
  ...
  async settleTerminal(snapshot){
    ...
    if(this.isSolo()){
      ...
      const response=await this.accountRequest('/internal/game/settle',{gameId,sessionId:this.room.sessionId,mode:'solo',winnerPlayerId:winnerId,finalPoints,scores,computer:completedComputer,participants:[participant],history:this.adminGameHistory(),recordedAt:this.now()});
    ...
    const response=await this.accountRequest('/internal/game/settle',{gameId,sessionId:this.room.sessionId,mode:'online',winnerPlayerId:winnerId,finalPoints,scores,participants,history:this.adminGameHistory(),recordedAt:this.now()});
