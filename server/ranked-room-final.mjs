...
export class FinalRankedRoomCore extends RankedRoomCore{
  ...
  adminGameHistory(){
    if(!this.room?.matchId)return null;
    const match=this.authority.exportMatch(this.room.matchId),startRevision=Math.max(0,Number(this.room.currentGameStartRevision)||0);
    return {roomCode:this.room.roomCode,matchId:match.id,gameSequence:this.room.gameSequence||0,startRevision,endRevision:match.revision,createdAt:match.createdAt,completedAt:match.completedAt,seatByPlayer:match.seatByPlayer,events:(match.events||[]).filter(event=>(Number(event.revision)||0)>startRevision),actions:match.actions||{},finalState:match.state};
  }
  async settleTerminal(snapshot){
    ...
    const terminal=snapshot.terminalResult,winnerId=terminal.winnerId||null,engineTerminal=terminal.result||{},settlement=engineTerminal.settlement||terminal.settlement||null,finalPoints=Math.max(0,Math.trunc(Number(engineTerminal.score??engineTerminal.finalPoints??terminal.finalPoints??0))),milestones=this.milestonesForCurrentGame(),scores=this.rankedScores(finalPoints,winnerId),settlementReasons=Array.isArray(settlement?.reasons)?[...settlement.reasons]:[],formulaSteps=Array.isArray(settlement?.formulaSteps)?[...settlement.formulaSteps]:[];if(finalPoints&&formulaSteps.at(-1)!==`Final ${finalPoints}`)formulaSteps.push(`Final ${finalPoints}`);this.room.settledGameIds.push(gameId);this.room.sessionStats.gamesPlayed=(Number(this.room.sessionStats.gamesPlayed)||0)+1;
    ...
    if(this.isSolo()){
      ...
      const completedComputer={playerId:bot.playerId,nickname:bot.nickname,level:bot.computerLevel,walletAfter:bot.walletCoins,won:winnerId===bot.playerId,walletDelta:winnerId===bot.playerId?finalPoints:winnerId===user.playerId?-finalPoints:0,coinsWon:winnerId===bot.playerId?finalPoints:0,points:scores[bot.playerId]?.points||0,rawScore:scores[bot.playerId]?.rawScore||0,milestones:milestones[bot.playerId]||{}};
      ...
      const response=await this.accountRequest('/internal/game/settle',{gameId,sessionId:this.room.sessionId,mode:'solo',winnerPlayerId:winnerId,finalPoints,scores,computer:completedComputer,settlementType:'normal',settlementReasons,formulaSteps,participants:[participant],history:this.adminGameHistory(),recordedAt:this.now()});
    ...
    const response=await this.accountRequest('/internal/game/settle',{gameId,sessionId:this.room.sessionId,mode:'online',winnerPlayerId:winnerId,finalPoints,scores,settlementType:'normal',settlementReasons,formulaSteps,participants,history:this.adminGameHistory(),recordedAt:this.now()});
