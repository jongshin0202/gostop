...
export function parseClientMessage(input){
  ...
  if(!['action','syncRequest','ping','friendlyReferral'].includes(message.type))throw Object.assign(new Error('Unknown message type.'),{code:'MALFORMED_MESSAGE'});
  ...
  if(message.type==='friendlyReferral'&&(!['offerShown','signupStarted','declined'].includes(message.status)||!['game10','session-end'].includes(message.stage)))throw Object.assign(new Error('Friendly referral message is malformed.'),{code:'MALFORMED_MESSAGE'});
