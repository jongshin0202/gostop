...
export class Lobby{
  ...
  candidateClients(client,{autoOnly=false}={}){const unique=new Map();for(const candidate of this.clients.values()){if(candidate===client||candidate.account.id===client.account.id||candidate.available===false)continue;if(autoOnly&&!candidate.autoMatching)continue;if(!autoOnly&&candidate.autoMatching)continue;if(!unique.has(candidate.account.id))unique.set(candidate.account.id,candidate);}return [...unique.values()];}
  ...
  async search(client,query,rows=null){rows=rows||await this.leaderboardRows();const needle=String(query||'').trim().toLowerCase(),me=this.profile(client,rows);return this.candidateClients(client).map(candidate=>this.profile(candidate,rows)).filter(profile=>profile.nickname.toLowerCase().includes(needle)).map(profile=>({...profile,similarity:similarityPercent(me,profile)})).sort((a,b)=>{const ax=a.nickname.toLowerCase()===needle?0:1,bx=b.nickname.toLowerCase()===needle?0:1;return ax-bx||distance(me,a)-distance(me,b)||a.nickname.localeCompare(b.nickname);}).slice(0,MAX_LOBBY_RESULTS);}
  async broadcastRecommendations(){const rows=await this.leaderboardRows();for(const client of this.clients.values()){if(client.available===false)continue;this.send(client.socket,{type:'recommendations',players:await this.recommendations(client,rows),onlineCount:this.candidateClients(client).length,autoMatching:!!client.autoMatching});}}
  createAcceptedMatch(creator,opponent,rows,{automatic=false}={}){const now=Date.now(),id=randomId(this.crypto,automatic?'match':'challenge'),challenge={id,from:creator.account.id,to:opponent.account.id,status:'accepted',automatic,createdAt:now,acceptedAt:now,expiresAt:now+ACCEPTED_CHALLENGE_TTL_MS};this.challenges.set(id,challenge);creator.autoMatching=false;opponent.autoMatching=false;creator.available=false;opponent.available=false;this.send(creator.socket,{type:'challengeAcceptedCreateRoom',requestId:id,automatic,opponent:this.profile(opponent,rows)});this.send(opponent.socket,{type:'challengeAcceptedWaiting',requestId:id,automatic,opponent:this.profile(creator,rows)});return challenge;}
  async tryAutoMatch(client){if(!client.autoMatching||client.available===false)return false;const rows=await this.leaderboardRows();if(!client.autoMatching||client.available===false)return false;const candidates=this.candidateClients(client,{autoOnly:true});if(!candidates.length){this.send(client.socket,{type:'autoMatchWaiting'});await this.broadcastRecommendations();return false;}const me=this.profile(client,rows),partner=candidates.map(candidate=>({candidate,profile:this.profile(candidate,rows)})).sort((a,b)=>distance(me,a.profile)-distance(me,b.profile)||b.profile.gamesPlayed-a.profile.gamesPlayed||a.profile.nickname.localeCompare(b.profile.nickname))[0].candidate;if(!partner?.autoMatching||partner.available===false||!client.autoMatching||client.available===false)return this.tryAutoMatch(client);const partnerProfile=this.profile(partner,rows);this.send(client.socket,{type:'autoMatchFound',opponent:{...partnerProfile,similarity:similarityPercent(me,partnerProfile)}});this.send(partner.socket,{type:'autoMatchFound',opponent:{...me,similarity:similarityPercent(me,partnerProfile)}});this.createAcceptedMatch(client,partner,rows,{automatic:true});await this.broadcastRecommendations();return true;}
  ...
  async handle(client,data){
    ...
    if(message.type==='recommendations'){const rows=await this.leaderboardRows();this.send(client.socket,{type:'recommendations',players:await this.recommendations(client,rows),onlineCount:this.candidateClients(client).length,autoMatching:!!client.autoMatching});return;}
    if(message.type==='search'){const rows=await this.leaderboardRows();this.send(client.socket,{type:'searchResults',query:String(message.query||''),players:await this.search(client,message.query,rows),onlineCount:this.candidateClients(client).length,autoMatching:!!client.autoMatching});return;}
    if(message.type==='autoMatchStart'){client.available=true;client.autoMatching=true;await this.tryAutoMatch(client);return;}
    if(message.type==='autoMatchCancel'){client.autoMatching=false;this.send(client.socket,{type:'autoMatchCancelled'});await this.broadcastRecommendations();return;}
    if(message.type==='setAvailability'){client.available=message.available!==false;if(!client.available)client.autoMatching=false;await this.broadcastRecommendations();return;}
    if(message.type==='challenge'){
      if(client.autoMatching){client.autoMatching=false;}
      ...
      const target=this.clientByAccountId(message.accountId);if(!target||target.account.id===client.account.id||target.available===false||target.autoMatching)return this.send(client.socket,{type:'challengeError',code:'PLAYER_UNAVAILABLE',message:'That player is no longer available.'});
      ...
      this.send(client.socket,{type:'challengeSent',requestId:id,to:this.profile(target,rows)});await this.broadcastRecommendations();return;
    }
    if(message.type==='challengeResponse'){
      ...
      const challenger=this.clientByAccountId(challenge.from);if(!challenger){this.challenges.delete(challenge.id);await this.broadcastRecommendations();return this.send(client.socket,{type:'challengeError',code:'PLAYER_UNAVAILABLE',message:'The requesting player is no longer online.'});}
      if(!message.accept){this.challenges.delete(challenge.id);this.send(challenger.socket,{type:'challengeDeclined',requestId:challenge.id,by:{accountId:client.account.id,nickname:client.account.nickname}});await this.broadcastRecommendations();return;}
      const rows=await this.leaderboardRows();challenger.autoMatching=false;client.autoMatching=false;challenger.available=false;client.available=false;challenge.status='accepted';challenge.acceptedAt=Date.now();challenge.expiresAt=Date.now()+ACCEPTED_CHALLENGE_TTL_MS;
      this.send(challenger.socket,{type:'challengeAcceptedCreateRoom',requestId:challenge.id,opponent:this.profile(client,rows)});this.send(client.socket,{type:'challengeAcceptedWaiting',requestId:challenge.id,opponent:this.profile(challenger,rows)});await this.broadcastRecommendations();return;
    }
    if(message.type==='challengeRoomReady'){
      ...
      const target=this.clientByAccountId(challenge.to);if(!target){client.available=true;this.challenges.delete(challenge.id);await this.broadcastRecommendations();return this.send(client.socket,{type:'challengeError',code:'PLAYER_UNAVAILABLE',message:'The other player went offline before joining.'});}
  ...
  async disconnect(socket){const client=this.clients.get(socket);if(!client)return;this.clients.delete(socket);for(const [id,challenge] of this.challenges){if(challenge.from===client.account.id||challenge.to===client.account.id){const otherId=challenge.from===client.account.id?challenge.to:challenge.from,other=this.clientByAccountId(otherId);if(other){other.available=true;other.autoMatching=false;this.send(other.socket,{type:'challengeCancelled',requestId:id,message:'The other player went offline.'});}this.challenges.delete(id);}}await this.broadcastRecommendations();}
  async fetch(request){
