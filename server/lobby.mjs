...
export class Lobby{
  ...
  async acceptAutoMatchCandidate(client,requestedAccountId=null){
    ...
    if(!target){client.autoMatching=false;client.autoMatchCandidateId=null;const liveTargets=targetClients.filter(item=>this.clientPresence(item).heartbeatFresh),message=`DEBUG target-lookup: targetFound=false targetSockets=${targetClients.length} liveTargetSockets=${liveTargets.length} targetBusy=${this.accountTwoPlayerBusy(accountId)}`;console.log('AUTO_MATCH_ACCEPT_REJECTED',JSON.stringify({stage:'target-lookup',reason:'no-challengeable-target'}));this.send(client.socket,{type:'challengeDeclined',requestId:null,by:{accountId:accountId,nickname:message}});return false;}
    ...
    if(!challenge){const liveCreatorTwoPlayer=creatorClients.filter(item=>item.twoPlayer&&this.clientPresence(item).heartbeatFresh),creatorModes=liveCreatorTwoPlayer.map(item=>item.mode||'unknown').join(',')||'none',message=`DEBUG start-challenge: ${reasons.join(',')||'unknown'} creatorSockets=${creatorClients.length} liveTwoPlayer=${liveCreatorTwoPlayer.length} modes=${creatorModes} targetSockets=${targetClients.length}`;console.log('AUTO_MATCH_ACCEPT_REJECTED',JSON.stringify({stage:'start-challenge',reasons}));client.autoMatching=false;this.send(client.socket,{type:'challengeDeclined',requestId:null,by:{accountId:accountId,nickname:message}});return false;}
