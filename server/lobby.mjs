...
export class Lobby{
  ...
  clientByAccountId(accountId,{challengeableOnly=false,activeOnly=false}={}){
    const clients=this.clientsForAccount(accountId).slice().sort((a,b)=>{const ap=this.clientPresence(a),bp=this.clientPresence(b),ar=ap.active?0:ap.notifyable?1:2,br=bp.active?0:bp.notifyable?1:2;return ar-br||(Number(b.lastActivityAt)||0)-(Number(a.lastActivityAt)||0);});
  ...
  presenceForAccount(accountId){
    ...
    if(away)return {online:true,challengeable:notifyable&&!this.pendingChallengeFor(accountId),status:'away',mode:away.mode||'menu',notificationsEnabled:notifyable};
  ...
  challengeParticipantsAvailable(challenge){const from=this.clientById(challenge.fromClientId),to=this.clientById(challenge.toClientId);return !!from&&!!to&&!from.twoPlayer&&!to.twoPlayer&&this.clientCanReceiveChallenge(to);}
  startChallenge(creator,target,rows,{automatic=false}={}){
    const creatorPresence=this.clientPresence(creator);if(!creator||!target||creator.account.id===target.account.id||!creatorPresence.active||!this.clientCanReceiveChallenge(target)||this.accountTwoPlayerBusy(creator.account.id)||this.accountTwoPlayerBusy(target.account.id)||this.activeChallengeFor(creator.account.id)||this.activeChallengeFor(target.account.id))return null;
    const now=Date.now(),id=randomId(this.crypto,'challenge'),challenge={id,from:creator.account.id,to:target.account.id,fromClientId:creator.clientId,toClientId:target.clientId,status:'pending',automatic,createdAt:now,expiresAt:now+CHALLENGE_TTL_MS};this.challenges.set(id,challenge);creator.autoMatching=!!automatic;
    const fromProfile=this.profile(creator,rows),toProfile=this.profile(target,rows);
    this.send(target.socket,{type:'playRequest',requestId:id,automatic,expiresInSeconds:60,from:fromProfile});
  ...
  async tryAutoMatch(client){
    if(!this.clientPresence(client).active||client.twoPlayer||this.activeChallengeFor(client.account.id))return false;
    const rows=await this.leaderboardRows(),candidates=this.candidateClients(client,{activeOnly:true});
  ...
  async tryWaitingAutoMatches(){
    ...
    for(const client of this.clients.values()){
      ...
      if(!client.autoMatching||!this.clientPresence(client).active||client.twoPlayer||this.activeChallengeFor(client.account.id))continue;
