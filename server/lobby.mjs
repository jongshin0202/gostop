...
function normalize(value,minimum){const n=Math.abs(Number(value)||0);return Math.max(minimum,n);}
...
export class Lobby{
  ...
  async resolveAccount(token){if(!token)return null;const response=await this.accountStore().fetch(new Request('https://accounts/internal/resolve',{headers:{Authorization:`Bearer ${token}`}}));if(!response.ok)return null;return (await response.json()).account||null;}
  ...
  profile(client,rows){const row=rows.get(String(client.account.nickname||'').toLowerCase())||{};return {accountId:client.account.id,nickname:client.account.nickname,score:Number(row.score)||0,gamesPlayed:Number(row.gamesPlayed)||0,walletCoins:Number(client.account.walletCoins)||0,provisional:!!row.provisional,online:true};}
  ...
  pruneChallenges(){const now=Date.now();for(const [id,challenge] of this.challenges)if(challenge.expiresAt<=now)this.challenges.delete(id);}
  async allocateRoom(challenger,target){
    for(let attempt=0;attempt<5;attempt++){
      const code=roomCode(this.crypto),stub=this.env.GAME_ROOMS.get(this.env.GAME_ROOMS.idFromName(code));
      const created=await stub.fetch(new Request('https://room/initialize',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({roomCode:code,account:challenger.account})}));
      if(created.status===409)continue;
      if(!created.ok)throw new Error('Could not create game room.');
      const hostRoom=(await created.json()).room;
      const joined=await stub.fetch(new Request('https://room/join',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({account:target.account})}));
      if(!joined.ok)throw new Error('Could not join game room.');
      const guestRoom=(await joined.json()).room;
      return {hostRoom,guestRoom};
    }
    throw new Error('Could not allocate game room.');
  }
  async handle(client,data){
    ...
    if(message.type==='challenge'){
      ...
      const rows=await this.leaderboardRows(),id=randomId(this.crypto,'challenge'),challenge={id,from:client.account.id,to:target.account.id,createdAt:now,expiresAt:now+CHALLENGE_TTL_MS};this.challenges.set(id,challenge);
    ...
    if(message.type==='challengeResponse'){
      const challenge=this.challenges.get(message.requestId);if(!challenge||challenge.to!==client.account.id)return this.send(client.socket,{type:'challengeError',code:'REQUEST_EXPIRED',message:'That play request has expired.'});
      this.challenges.delete(challenge.id);const challenger=this.clientByAccountId(challenge.from);if(!challenger)return this.send(client.socket,{type:'challengeError',code:'PLAYER_UNAVAILABLE',message:'The requesting player is no longer online.'});
      if(!message.accept){this.send(challenger.socket,{type:'challengeDeclined',requestId:challenge.id,by:{accountId:client.account.id,nickname:client.account.nickname}});return;}
      try{challenger.available=false;client.available=false;const rooms=await this.allocateRoom(challenger,client);this.send(challenger.socket,{type:'challengeAccepted',requestId:challenge.id,room:rooms.hostRoom});this.send(client.socket,{type:'challengeAccepted',requestId:challenge.id,room:rooms.guestRoom});}
      catch(error){challenger.available=true;client.available=true;this.send(challenger.socket,{type:'challengeError',code:'ROOM_CREATE_FAILED',message:error.message});this.send(client.socket,{type:'challengeError',code:'ROOM_CREATE_FAILED',message:error.message});}
      return;
    }
  }
  disconnect(socket){const client=this.clients.get(socket);if(!client)return;this.clients.delete(socket);for(const [id,challenge] of this.challenges){if(challenge.from===client.account.id||challenge.to===client.account.id){const otherId=challenge.from===client.account.id?challenge.to:challenge.from,other=this.clientByAccountId(otherId);if(other)this.send(other.socket,{type:'challengeCancelled',requestId:id,message:'The other player went offline.'});this.challenges.delete(id);}}}
  async fetch(request){
    ...
    const protocol=request.headers.get('Sec-WebSocket-Protocol')?.split(',').map(value=>value.trim()).find(value=>value.startsWith('gostop-auth.'));const token=protocol?.slice('gostop-auth.'.length),account=await this.resolveAccount(token);if(!account)return json({ok:false,error:{code:'AUTH_REQUIRED',message:'Login required.'}},401);
    const pair=new WebSocketPair(),clientSocket=pair[0],serverSocket=pair[1];serverSocket.accept();const client={socket:serverSocket,account:clone(account),available:true};this.clients.set(serverSocket,client);serverSocket.addEventListener('message',event=>this.handle(client,event.data));serverSocket.addEventListener('close',()=>this.disconnect(serverSocket));serverSocket.addEventListener('error',()=>this.disconnect(serverSocket));this.send(serverSocket,{type:'connected',account:{id:account.id,nickname:account.nickname,walletCoins:account.walletCoins}});this.send(serverSocket,{type:'recommendations',players:await this.recommendations(client)});return new Response(null,{status:101,webSocket:clientSocket,headers:{'Sec-WebSocket-Protocol':protocol}});
  }
}

export {distance,CHALLENGE_TTL_MS};
