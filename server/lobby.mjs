...
export class Lobby{
  ...
  profile(client,rows){const row=rows.get(String(client.account.nickname||'').toLowerCase())||{},gamesPlayed=finite(row.gamesPlayed),wins=finite(row.wins),totalCoinsEarned=finite(row.totalCoins),rate=Number.isFinite(Number(row.score))?Number(row.score):(gamesPlayed?totalCoinsEarned/gamesPlayed:0),presence=this.presenceForAccount(client.account.id);return {accountId:client.account.id,nickname:client.account.nickname,score:rate,coinsPerGame:rate,totalCoinsEarned,gamesPlayed,wins,losses:Math.max(0,finite(row.losses)||(gamesPlayed-wins)),walletCoins:finite(client.account.walletCoins),rank:Number(row.rank)||null,provisional:!!row.provisional,countryCode:client.account.countryCode||row.countryCode||null,regionCode:client.account.regionCode||row.regionCode||null,...presence};}
  ...
  async search(client,query,rows=null){
    ...
    return directory.filter(player=>player.accountId!==client.account.id).map(player=>{
      const presence=this.presenceForAccount(player.accountId),gamesPlayed=finite(player.gamesPlayed),wins=finite(player.wins),profile={...player,totalCoinsEarned:finite(player.totalCoins),coinsPerGame:finite(player.score),score:finite(player.score),gamesPlayed,wins,losses:Math.max(0,finite(player.losses)||(gamesPlayed-wins)),walletCoins:finite(player.walletCoins),...presence};
