...
export class Lobby{
  ...
  async acceptAutoMatchCandidate(client,requestedAccountId=null){
    ...
    const target=this.clientByAccountId(accountId,{challengeableOnly:true}),now=Date.now(),fmt=item=>{const p=this.clientPresence(item,now),tab=item.tabId?String(item.tabId).slice(0,8):'NONE',age=Math.max(0,Math.round((now-(Number(item.lastPresenceAt)||now))/1000));return `tab=${tab} tp=${item.twoPlayer?'Y':'N'} mode=${item.mode||'none'} fg=${item.foreground?'Y':'N'} hb=${age}s fresh=${p.heartbeatFresh?'Y':'N'}`;};
    if(!target){client.autoMatching=false;client.autoMatchCandidateId=null;const targetSockets=this.clientsForAccount(accountId),message=`DEBUG target lookup | currentTab=${client.tabId?String(client.tabId).slice(0,8):'NONE'} | targetSockets=${targetSockets.length} | ${targetSockets.map(fmt).join(' || ')||'none'}`;this.send(client.socket,{type:'challengeDeclined',requestId:null,by:{accountId,nickname:message}});return false;}
    ...
    if(!challenge){const creatorSockets=this.clientsForAccount(client.account.id),targetSockets=this.clientsForAccount(accountId),reasons=[];if(!this.clientCanReceiveChallenge(client))reasons.push('creator-not-challengeable');if(!this.clientCanReceiveChallenge(target))reasons.push('target-not-challengeable');if(this.accountTwoPlayerBusy(client.account.id))reasons.push('creator-account-busy');if(this.accountTwoPlayerBusy(accountId))reasons.push('target-account-busy');if(this.activeChallengeFor(client.account.id))reasons.push('creator-active-challenge');if(this.activeChallengeFor(accountId))reasons.push('target-active-challenge');client.autoMatching=false;const message=`DEBUG ${reasons.join(',')||'unknown'} | currentTab=${client.tabId?String(client.tabId).slice(0,8):'NONE'} | creator[${creatorSockets.length}]=${creatorSockets.map(fmt).join(' || ')} | target[${targetSockets.length}]=${targetSockets.map(fmt).join(' || ')}`;this.send(client.socket,{type:'challengeDeclined',requestId:null,by:{accountId,nickname:message}});return false;}
    return true;
  }
