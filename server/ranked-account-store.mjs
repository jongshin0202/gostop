...
export class AccountStore extends BaseAccountStore{
  ...
  async forceQuit(request){
    ...
    const accountNotice=firstOfMonth&&settlementType!=='nagari'?{id:noticeId('disconnect-forgiven',gameId),type:'disconnect-forgiven',gameId,fairPoints,settlementType,recordedAt,month,firstOfMonth:true,reason:body.reason}:null;if(accountNotice)account.pendingNotices=[...(account.pendingNotices||[]).filter(item=>item.id!==accountNotice.id),accountNotice];
