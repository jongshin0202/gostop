...
export class AccountStore extends BaseAccountStore{
  ...
  async forceQuit(request){
    ...
    if(settlementType!=='nagari'&&!firstOfMonth){const quitterStats=ensureStats(account,month);for(const stats of quitterStats){stats.gamesPlayed+=1;stats.losses=(Number(stats.losses)||0)+1;}addMilestones(quitterStats,body.quitterMilestones);}
    ...
    const record={...body,gameId,type:'abandonment',settlementType,fairPoints,penaltyCoins:penalty,opponentRewardCoins:rewardCoins,firstOfMonth,recordedAt,account:publicAccount(account),opponent:publicOpponent,adminConnections:{quitter:account.lastConnection?{...account.lastConnection}:null,opponent:opponent?.lastConnection?{...opponent.lastConnection}:null},adminLocations:{quitter:account.location?{...account.location}:null,opponent:opponent?.location?{...opponent.location}:null},accountNotice,opponentNotice};await this.storage.put(`forceQuit:${gameId}`,record);await this.storage.put(`game:${gameId}`,record);
