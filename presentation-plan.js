(() => {
  'use strict';
  function planOnlinePresentation(events,initialStages={}){
    ...
    for(const event of events){
      ...
      if(event.type==='cardPlayed'){
        ...
        if((event.targetId||event.matchCount===0)&&stages.get(event.card.id)==='waitingTarget'){steps.push({kind:'stageSlap',cardId:event.card.id,event});stages.set(event.card.id,'landed');}
  ...
  const api=Object.freeze({planOnlinePresentation});globalThis.GoStopPresentationPlan=api;if(typeof module!=='undefined'&&module.exports)module.exports=api;
})();
