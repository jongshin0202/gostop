...
test('deployment configuration uses a public Vercel build-time URL and declarative SQLite Durable Object export',()=>{
  ...
  assert.match(wrangler,/\[exports\.GameRoom\]/);assert.match(wrangler,/type = "durable-object"/);assert.match(wrangler,/storage = "sqlite"/);assert.match(wrangler,/INACTIVITY_NUDGE_SECONDS = "180"/);assert.match(wrangler,/INACTIVITY_NUDGE_PHASE_SECONDS = "60"/);assert.match(wrangler,/ABANDONMENT_COUNTDOWN_SECONDS = "30"/);assert.match(wrangler,/PAUSE_DURATION_SECONDS = "180"/);assert.doesNotMatch(wrangler,/ABANDONMENT_TIMEOUT_SECONDS/);assert.doesNotMatch(wrangler,/\[\[durable_objects\.exports\]\]/);assert.doesNotMatch(wrangler,/\[\[migrations\]\]/);
});