...
test('deployment configuration uses a public Vercel build-time URL and declarative SQLite Durable Object export',()=>{
  ...
  assert.match(wrangler,/\[exports\.GameRoom\]/);assert.match(wrangler,/type = "durable-object"/);assert.match(wrangler,/storage = "sqlite"/);assert.match(wrangler,/ABANDONMENT_TIMEOUT_SECONDS = "180"/);assert.doesNotMatch(wrangler,/\[\[durable_objects\.exports\]\]/);assert.doesNotMatch(wrangler,/\[\[migrations\]\]/);
});