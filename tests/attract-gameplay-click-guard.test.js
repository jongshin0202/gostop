...
test('attract capture can never swallow gameplay clicks once an online session exists',()=>{
  assert.match(ranked,/if\(!attractMode\|\|leaderboardScreen\.hidden\|\|globalThis\.goStopOnlineSession\)return;if\(Date\.now\(\)<attractSwipeSuppressClickUntil\)\{event\.preventDefault\(\);event\.stopPropagation\(\);return;\}event\.preventDefault\(\);event\.stopPropagation\(\)/);
