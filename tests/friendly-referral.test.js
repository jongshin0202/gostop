...
test('Friendly Play With Friend offers signup after game ten and session end, then delays inviter reward until ten outside Competitive games',()=>{
  ...
  assert.match(ranked,/function friendlyGuestContext\(snapshot=null\)\{[^]*String\(context\.roomCode\|\|''\)\.toUpperCase\(\)!==roomCode\)return null;return context;\}/);assert.doesNotMatch(ranked,/snapshot&&snapshot\.seatId!=='playerB'/);
  assert.match(ranked,/context\.signupStarted\|\|context\.sessionEndOffered\)return friendlySignupDialog\.open\|\|friendlyGuestMessageDialog\.open\|\|authDialog\.open\|\|verificationDialog\.open\|\|successDialog\.open/);
  assert.match(app,/resumeFriendlyRoom\(roomCode\)/);assert.match(app,/gostop-friendly-friend-joined/);assert.match(app,/handleFriendlyTerminal\?\.\(snapshot\)/);assert.match(app,/handleFriendlySessionEnd\?\.\(snapshot\)/);assert.match(app,/flow\.forceEnded\?t\('friendForceEnded'\):t\('friendEnded'\)/);assert.doesNotMatch(app,/onlineAnonymousMode&&!flow\.forceEnded&&globalThis\.GoStopRanked\?\.handleFriendlySessionEnd/);assert.match(app,/opponentEndedOkBtn\.addEventListener[^]*handleFriendlySessionEnd\?\.\(snapshot\)/);assert.match(online,/sendFriendlyReferral\(status,stage\)/);
});
