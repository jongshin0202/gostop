...
test('protected first disconnect waives Coins but still records the quitter loss and opponent win',async()=>{
  const clock='2026-09-20T04:45:00.000Z',store=new AccountStore({storage:new MemoryStorage()},{},{cryptoApi:webcrypto,now:()=>clock}),q=await registered(store,'protected-loss@example.com','ProtectedLoser'),w=await registered(store,'protected-win@example.com','ProtectedWinner');
  const result=await (await store.fetch(post('/internal/force-quit',{gameId:'protected-loss-1',sessionId:'protected-session',mode:'online',accountId:q.account.id,opponentAccountId:w.account.id,fairPoints:19,settlementType:'current-settlement',reason:'disconnect-timeout',recordedAt:clock}))).json();
  assert.equal(result.firstOfMonth,true);assert.equal(result.penaltyCoins,0);
  const quitter=await store.accountById(q.account.id),winner=await store.accountById(w.account.id);
  assert.equal(quitter.stats.global.gamesPlayed,1);assert.equal(quitter.stats.global.losses,1);assert.equal(quitter.stats.global.wins,0);
  assert.equal(winner.stats.global.gamesPlayed,1);assert.equal(winner.stats.global.wins,1);assert.equal(winner.stats.global.losses,0);
  assert.equal(quitter.walletCoins,200);
});
