...
test('ranked Solo disconnect freezes the five-point settlement and stops the computer during grace',async()=>{
  ...
  // Secure-random Solo setup can let the computer collect scoring cards before this synthetic
  // disconnect scenario is injected. Normalize only the opponent scoring inputs owned by this test.
  record.state[botSide].captured=[];record.state[botSide].gukjinMode='animal';record.state[botSide].firstPpeokPoints=0;
